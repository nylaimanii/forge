<script lang="ts">
	...
	function warnSave() {
		showToast('sign up to save changes — this is a demo', 'info');
	}
</script>
...
>
	...
	>
		{#each tables as table}
			...
			>
				...
				<!-- add field (shows sign-up prompt) -->
				<button
					onclick={warnSave}
					class="w-full px-3 py-2 flex items-center gap-1.5
						text-[var(--color-muted)] hover:text-[var(--color-electric)]
						text-xs font-[var(--font-ui)] border-t border-[var(--color-border)]
						transition-colors"
				>
					<Plus size={10} />
					add field
	...
	<div class="absolute bottom-4 right-4 flex flex-col gap-1 z-10">
		<button
			onclick={() => { zoom = Math.min(2, zoom * 1.2); }}
		...
		<button
			onclick={() => { zoom = Math.max(0.3, zoom * 0.8); }}
			class="w-8 h-8 rounded-lg glass border border-[var(--color-border)] flex items-center justify-center text-[var(--color-muted)] hover:text-[var(--color-text)] transition-colors"
			aria-label="Zoom out"
	...
	<div class="absolute bottom-4 left-4 z-10">
		<button
			onclick={warnSave}
			class="flex items-center gap-2 px-3 h-8 rounded-lg
				bg-[var(--color-surface-2)] border border-[var(--color-border)]
				text-[var(--color-muted)] hover:text-[var(--color-electric)]
				text-xs font-[var(--font-ui)] transition-colors"
		>
			<Plus size={12} />
			add table
		</button>
